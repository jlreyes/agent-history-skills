> **ARCHIVED SNAPSHOT — Codex CLI 0.137.x (verified Sept 2025 → June 2026).**
> This is a historical copy of `data-model.md` as it stood for the 0.137.x line.
> It is kept to help read *old* rollout files written by 0.137.x and earlier.
> For the current on-disk format, see the live [data-model.md](../data-model.md).
> Known deltas since this snapshot (introduced by ~0.140–0.142): `session_meta`
> gained a `session_id` field alongside `id`; the `RolloutItem` envelope gained an
> `inter_agent_communication` variant; `turn_context` gained many fields
> (`turn_id`, `workspace_roots`, `current_date`, `timezone`, `permission_profile`,
> `collaboration_mode`, `personality`, `multi_agent_version`, `summary`, …);
> `compacted` gained window-chain ids; `state_5.sqlite` gained `recency_at`/`recency_at_ms`.
